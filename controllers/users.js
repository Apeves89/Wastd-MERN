...
async function signup(req, res) {
  ...
  const filePath = `wastd/${uuidv4()}-${req.file.originalname}`;
  ...
  s3.upload(params, async function(err, data){
    ...
    const user = await User.create({...req.body, photoUrl: data.Location}); // data.Location is the is the url of our image on AWS
    try {
...
async function login(req, res) {
  ...
  try {
    ...
    if (!match) return res.status(401).json({err: 'bad credentials'});;
    const token = createJWT(user);
    res.json(token);
  } catch (err) {
